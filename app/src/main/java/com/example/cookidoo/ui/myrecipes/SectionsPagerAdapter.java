package com.example.cookidoo.ui.myrecipes;

import android.content.Context;

import androidx.annotation.Nullable;
import androidx.annotation.StringRes;
import androidx.fragment.app.Fragment;
import androidx.fragment.app.FragmentManager;
import androidx.fragment.app.FragmentPagerAdapter;

import com.example.cookidoo.ui.myrecipes.all.AllFragment;
import com.example.cookidoo.R;
import com.example.cookidoo.ui.myrecipes.created.CreatedFragment;
import com.example.cookidoo.ui.myrecipes.saved.SavedFragment;

/**
 * A [FragmentPagerAdapter] that returns a fragment corresponding to
 * one of the sections/tabs/pages.
 */
public class SectionsPagerAdapter extends FragmentPagerAdapter {

    @StringRes
    private static final int[] TAB_TITLES = new int[]{R.string.my_recipes_tab_all, R.string.my_recipes_tab_created, R.string.my_recipes_tab_saved};
    private final Context mContext;

    public SectionsPagerAdapter(Context context, FragmentManager fm) {
        super(fm);
        mContext = context;
    }

    @Override
    public Fragment getItem(int position) {
        // getItem is called to instantiate the fragment for the given page.
        // Return a PlaceholderFragment (defined as a static inner class below).
        if (position == 0) {
            return AllFragment.newInstance(position + 1);
        } else if (position == 1) {
            return CreatedFragment.newInstance(position + 1);
        } else {
            return SavedFragment.newInstance(position + 1);
        }
    }


    @Nullable
    @Override
    public CharSequence getPageTitle(int position) {
        return mContext.getResources().getString(TAB_TITLES[position]);
    }

    @Override
    public int getCount() {
        // Show 2 total pages.
        return 3;
    }
}